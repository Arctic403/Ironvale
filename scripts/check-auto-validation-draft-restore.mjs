...
assert(auto.indexOf('draftBaseline = captureDraftBaseline()') < auto.indexOf('state.baselineIntegrity = await window.RiftSurvivalValidatorGuard.captureIntegrity()'), 'draft baseline must be captured immediately before integrity baseline');
assert(auto.indexOf("if (!restoreDraftBaseline(draftBaseline)) throw new Error('Unable to restore exact terrain draft baseline')") < auto.indexOf('state.finalIntegrity = await window.RiftSurvivalValidatorGuard.captureIntegrity()'), 'exact draft must be restored before final integrity capture');
...
console.log('RiftSurvival auto-validation exact local draft restoration verified.');
