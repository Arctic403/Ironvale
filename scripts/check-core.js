...
if (world.terrain?.size?.[0] !== 5120 || world.terrain?.size?.[1] !== 5120) failures.push('terrain must be 5120x5120');
if (world.terrain?.sampleSpacing !== 5) failures.push('large-world authoritative terrain must use 5m samples');
if (world.terrain?.chunkSize !== 80 || world.terrain?.sectionSize !== 80) failures.push('terrain sections must be 80m');
if (world.terrain?.componentSize !== 160) failures.push('terrain components must be 160m');
if (world.terrain?.generator?.id !== 'island-v3' || world.terrain?.generator?.version !== 3) failures.push('island-v3 generator contract');
if (!Number.isFinite(Number(world.terrain?.seed)) || world.metadata?.deterministicSeededTerrain !== true || world.metadata?.terrainGeneratorVersion !== 2) failures.push('deterministic terrain seed/version metadata');
...
if (world.metadata?.largeWorldStreaming !== true || world.metadata?.worldSizeMeters !== 5120 || world.metadata?.authoritativeHeightfieldSpacingMeters !== 5) failures.push('large-world streaming metadata');
if (world.terrain?.landscape?.streaming?.componentRadius !== 2) failures.push('large-world resident component radius');
...
if (!app.includes("rift-survival:terrain:draft:v5") || !app.includes('serializeLandscapeEdits') || !app.includes('captureEditState')) failures.push('layer-aware terrain persistence/undo');
...
if (!worker.includes("id: 'island-v3'") || !worker.includes('env.WORLD_SEED')) failures.push('backend-owned island-v3 seed contract');
if (!worker.includes('WORLD_MAX_X = 5120') || !worker.includes('WORLD_MAX_Z = 5120')) failures.push('backend terrain bounds');
