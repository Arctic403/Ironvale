...
try{
 ...
 ok(bank.building?.floor_height>=12,'Bank storey spacing must remain at least 12m.');
 ...
 ok(vault&&area(vault)>=450&&vault.clearHeight>=10,'Grand Vault must be at least 450m² with 10m clear height.');
 ok(hall&&area(hall)>=700&&hall.clearHeight>=10,'Grand Banking Hall must be at least 700m² with 10m clear height.');
 for(const space of i?.spaces||[]) ok(space.clearHeight>=8&&(space.clearRatio??1)>=.65,`${space.id} must remain a genuinely tall Bank room (>=8m clear).`);
 ...
 ok(r.document.bounds.min[1]===-12,`Downtown basement bound ${r.document.bounds.min[1]} != -12.`); ok(r.stats.boundsVolume<=3_000_000,'Downtown composed bounds exceed 3M cells.');
