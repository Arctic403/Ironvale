...
function renderJobs(root,data) {
  ...
  root.innerHTML=`
    ...
    ${data.history?.length?panel('Recent Shifts',`<div class="ledger-list">${data.history.slice(0,8).map(h=>`<div class="ledger-row"><div><strong>${escapeHtml(h.event_id||'shift')}</strong><small>${escapeHtml(dateTime(h.created_at))}</small></div><span>${money(h.pay)}</span><b>+${h.skill_xp} skill</b></div>`).join('')}</div>`,{eyebrow:'WORKPLACE EVENTS'}):''}
...
function renderProperties(root,data) {
  ...
  root.innerHTML=`
    ...
    <a class="rc-button" href="#property-portfolio" data-route="property-portfolio">Property upgrades & rental portfolio</a><div class="property-grid">${(data.properties||[]).map(prop=>{
...
function renderFactions(root,data) {
  ...
  root.innerHTML=`
    ...
    <a class="rc-button" href="#faction-shop" data-route="faction-shop">Faction Quartermaster</a>
