...
function renderCasino(root,data) {
  ...
  root.innerHTML=`
    <section class="casino-hero"><div><span class="eyebrow">THE MERIDIAN</span><h2>Casino Floor</h2><p>Fictional in-game chips only. The account and daily grant are server-owned; individual game engines are still being brought online.</p></div><div class="chip-balance"><span>CHIPS</span><strong>${chips}</strong><button class="rc-button primary" data-service-action="claim-daily">Claim ${data.dailyChipGrant||75} daily chips</button></div></section>
    <div class="casino-floor">${(data.games||[]).map((game,index)=>`<article class="casino-game game-${index}"><div class="game-mark">${['♠','◆','●','▥','♞'][index]||'◆'}</div><span class="eyebrow">${escapeHtml(String(game.status||'planned').toUpperCase())}</span><h3>${escapeHtml(game.name)}</h3><p>${game.status==='enabled'?'Server game available.':'Game table is prepared in the frontend; the server game engine is not enabled yet.'}</p><button class="rc-button" disabled>${game.status==='enabled'?'Play':'Coming online'}</button></article>`).join('')}</div>
    <div class="info-banner">The old V1 casino presentation is restored as a floor/catalog here without reintroducing client-generated wagering outcomes.</div>`;
