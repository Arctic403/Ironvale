...
function resultCard(result) {
  return `<section class="battle-result ${result.won?'win':'loss'}"><div class="battle-side"><span>YOU</span><strong>${result.won?'WIN':'LOSS'}</strong></div><div class="battle-center"><span class="eyebrow">${escapeHtml(result.opponentName||'Opponent')}</span><h3>${escapeHtml(result.text||'Combat resolved')}</h3><div><span>${result.rounds} rounds</span><span>${result.damageDealt} damage dealt</span><span>${result.damageTaken} damage taken</span><span>+${result.xpGain} XP</span></div></div><div class="battle-side opponent"><span>TARGET</span><strong>${result.won?'DEFEATED':'WINS'}</strong></div></section>`;
}
