...
function applyBox(cells,op,write){
  for(let y=op.min[1];y<=op.max[1];y++)for(let z=op.min[2];z<=op.max[2];z++)for(let x=op.min[0];x<=op.max[0];x++)write(x,y,z);
}
...
export function replayRiftBuildingCells(document){
  ...
  for(const op of document.ops||[]){
    ...
    if(type==='set'){
      const k=key(...op.at);
      if(solidState(document,op.state))cells.set(k,op.state);else cells.delete(k);
      continue;
    }
    if(!op.min||!op.max)continue;
    if(type==='fill_box')applyBox(cells,op,(x,y,z)=>{const k=key(x,y,z);if(solidState(document,op.state))cells.set(k,op.state);else cells.delete(k)});
    else if(type==='cut_box')applyBox(cells,op,(x,y,z)=>cells.delete(key(x,y,z)));
    else if(type==='hollow_box')applyBox(cells,op,(x,y,z)=>{const edge=x===op.min[0]||x===op.max[0]||y===op.min[1]||y===op.max[1]||z===op.min[2]||z===op.max[2];const k=key(x,y,z);if(edge&&solidState(document,op.state))cells.set(k,op.state);else if(!edge)cells.delete(k)});
  }
  return cells;
}
...
export function measureRiftInteriorClearance(authoring){
  ...
  const cells=replayRiftBuildingCells(document);
  const floors=authoring.semantics.floors||[];
  const reports=[];
  for(const space of interior.spaces||[]){
    const floorMeta=floors.find(item=>item.floor===space.floor);
    if(!floorMeta)continue;
    const floorY=floorMeta.localY;
    const next=floors.find(item=>item.floor===space.floor+1);
    const ceilingY=next?next.localY:document.bounds.max[1]+1;
    const rect=localRect(space);
    const heights=[];
    for(let z=rect.min[1];z<=rect.max[1];z++)for(let x=rect.min[0];x<=rect.max[0];x++){
      if(!cells.has(key(x,floorY,z)))continue;
      let clear=0;
      for(let y=floorY+1;y<ceilingY;y++){
        if(cells.has(key(x,y,z)))break;
        clear++;
      }
      heights.push(clear);
    }
    const declared=Number(space.clearHeight||0);
    const pass=heights.filter(v=>v>=declared).length;
    reports.push({
...
function clipOp(op,bounds){
  const type=String(op.op||'').toLowerCase();
  if(type==='set')return inside(bounds,op.at)?clone(op):null;
  if(!op.min||!op.max)return clone(op);
  const hit=boxIntersection({min:op.min,max:op.max},bounds);
  if(!hit)return null;
  const next=clone(op);next.min=hit.min;next.max=hit.max;return next;
}

export function buildRiftInspectionDocument(authoring,options={}){
  const mode=String(options.mode||'full').toLowerCase();
  const document=clone(authoring.document);
  const floors=authoring.semantics.floors||[];
  const floor=Number(options.floor)||0;
  let bounds=clone(document.bounds);
  if(floor){
    const meta=floors.find(item=>item.floor===floor);
    if(!meta)throw new Error(`Inspection floor ${floor} does not exist.`);
    const next=floors.find(item=>item.floor===floor+1);
    bounds.min[1]=meta.localY;
    bounds.max[1]=next?next.localY-1:document.bounds.max[1];
  }
  if(mode==='section'){
    const axis=String(options.sectionAxis||'z').toLowerCase()==='x'?0:2;
    const side=String(options.sectionSide||'low').toLowerCase()==='high'?'high':'low';
    const focus=inspectionFocus(authoring,options.focus);
    const fallback=focus?Math.round(focus.target[axis]):Math.round((bounds.min[axis]+bounds.max[axis])/2);
    const at=Number.isFinite(Number(options.sectionAt))?Math.round(Number(options.sectionAt)):fallback;
  ...
  document.ops=(document.ops||[]).map(op=>clipOp(op,bounds)).filter(Boolean).filter(op=>!(mode==='floor'&&String(op._semanticRole||'')==='roof'));
  document.id=`${document.id}-inspection-${mode}${floor?`-f${floor}`:''}`;
  document.name=`${document.name} · inspection ${mode}${floor?` · F${floor}`:''}`;
  return document;
}

export function inspectionFocus(authoring,focusId){
  if(!focusId)return null;
  const core=(authoring.semantics?.interior?.verticalCores||[]).find(item=>item.id===focusId);
  if(!core)return null;
  const cells=core.openingCellsLocal||[];
  const points=cells.length?cells.map(p=>[p[0],p[1],p[2]]):[core.local||core.at];
  const xs=points.map(p=>p[0]),ys=points.map(p=>p[1]||0),zs=points.map(p=>p[2]??p[1]);
...
export function createRiftInspectionReport(authoring){
  const clearance=measureRiftInteriorClearance(authoring);
  const interior=authoring.semantics?.interior;
  return{
    version:1,buildingId:authoring.semantics.buildingId,name:authoring.semantics.name,worldBounds:authoring.semantics.worldBounds,
    floors:(authoring.semantics.floors||[]).map(item=>({floor:item.floor,localY:item.localY,worldY:item.worldY})),
    clearance,
