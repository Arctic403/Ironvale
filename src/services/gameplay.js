...
async function getService(service, userId, env, deps, url) {
  switch(service) {
    ...
    default: return deps.json({ok:false,error:'Unknown service'},404);
  }
}
...
async function getBank(userId,env,deps){
  ...
  return {ok:true,account,tiers:BANK_INVESTMENT_TIERS,investments:investments.results||[],ledger:ledger.results||[]};
}
...
async function jobAction(body,userId,env,deps){
 ...
 if(body.action==='work'){
   ...
   const position=getJobPosition(job,Number(state.skill_level)); const pay=position.pay;
...
async function getEducation(userId,env,deps){
 const t=now(); await env.DB.prepare("UPDATE player_education SET status='completed',completed_at=? WHERE user_id=? AND status='studying' AND completes_at<=?").bind(t,userId,t).run();
...
async function gymAction(body,userId,env,deps){
 ...
 const mult=Number(program.multipliers[stat]||1); const gain=Math.max(1,Math.round(mult*(1+Math.min(10,streak)*.02)));
 await env.DB.batch([
...
// PROPERTIES
async function ensureStarterProperty(userId,env){await env.DB.prepare("INSERT OR IGNORE INTO player_properties(user_id,property_id,is_home,purchased_at) VALUES(?,'shack',1,?)").bind(userId,now()).run();}
...
async function getMarket(userId,env){const rows=await env.DB.prepare('SELECT * FROM player_market_positions WHERE user_id=?').bind(userId).all();return {ok:true,assets:MARKET_ASSETS.map(asset=>({...asset,price:getMarketPrice(asset)})),positions:rows.results||[]};}
async function marketAction(body,userId,env,deps){
 const asset=getMarketAsset(String(body.assetId||'')); if(!asset)return {ok:false,error:'Unknown market asset',status:404}; const qty=positiveInt(body.quantity,10000);if(!qty)return {ok:false,error:'Enter a valid quantity',status:400};const price=getMarketPrice(asset),total=price*qty,t=now();
