...
function App() {
  ...
  const nav: { id: Screen; label: string; icon: string }[] = [
    {id:"character",label:"Character",icon:"ð¤"},{id:"city",label:"City",icon:"ðï¸"},{id:"crimes",label:"Crimes",icon:"ðµï¸"},{id:"combat",label:"Combat",icon:"âï¸"},{id:"gym",label:"Gym",icon:"ðï¸"},{id:"jobs",label:"Jobs",icon:"ð¼"},{id:"items",label:"Items",icon:"ð"},{id:"missions",label:"Missions",icon:"ð"},{id:"education",label:"Education",icon:"ð"},{id:"property",label:"Property",icon:"ð "},{id:"market",label:"Market",icon:"ð"},{id:"faction",label:"Faction",icon:"ð¡ï¸"},{id:"awards",label:"Awards",icon:"ð"},
  ];
  const title = nav.find(n => n.id === g.currentScreen)?.label || "RiftCity";
  return <div className="app-shell">
    <header className="topbar"><div><strong>RIFTCITY</strong><span className="muted">CORE</span></div><div className="topstats"><span>LV {g.level}</span><span>ðµ {money(g.gameState.cash)}</span><span>ð¦ {money(g.gameState.bank)}</span><span>â¡ {g.gameState.energy}/{MAX_ENERGY}</span><span>ð§  {g.gameState.nerve}/{g.maxNerve}</span><span>â¤ï¸ {Math.floor(g.gameState.health)}/{g.maxHealth}</span><span>ð {Math.floor(g.gameState.happiness)}/{getProperty(g.gameState.ownedProperty)?.maxHappiness ?? 100}</span></div></header>
    <aside className="sidebar">{nav.map(n => <button className={g.currentScreen === n.id ? "nav active" : "nav"} onClick={() => g.setCurrentScreen(n.id)} key={n.id}><span>{n.icon}</span>{n.label}</button>)}<div className="side-bottom"><button onClick={g.randomEncounter}>ð² Random Encounter</button><button onClick={() => { if (confirm("Reset your RiftCity save?")) g.resetGame(); }}>â» Reset Save</button></div></aside>
    <main className="content"><div className="page-head"><div><div className="eyebrow">RIFTCITY / {g.gameState.currentLocation.toUpperCase()}</div><h1>{title}</h1></div><div className="bars"><div><small>XP {levelInfo.currentXp}/100</small><div className="bar"><i style={{ width: `${levelInfo.currentXp}%` }}/></div></div><div><small>Happiness {Math.floor(g.gameState.happiness)}</small><div className="bar"><i style={{ width: `${Math.min(100, g.gameState.happiness)}%` }}/></div></div></div></div>
      {g.gameState.jailUntil && <div className="alert jail">ð JAILED Â· {formatTime(timeLeft(g.gameState.jailUntil))} remaining</div>}
      {g.gameState.hospitalUntil && <div className="alert hospital">ð¥ HOSPITAL Â· {formatTime(timeLeft(g.gameState.hospitalUntil))} remaining</div>}
...
function Combat({g}: {g: ReturnType<typeof useRiftCity>}) {
  return <><Panel title="Player Search"><p>Select a real player-profile style opponent. Profiles show level, status, location, equipment, health and combat stats before you attack.</p><div className="grid two">{OPPONENTS.map(o => <div className="card" key={o.id}>
    ...
    <h3>{o.name}</h3><p>{o.title} Â· {o.location}</p>
    ...
    <Button onClick={() => g.attack(o)} disabled={Boolean(g.gameState.jailUntil || g.gameState.hospitalUntil) || g.gameState.energy<25}>Attack Â· 25 â¡</Button>
  </div>)}</div></Panel>
...
function Faction({g}: {g: ReturnType<typeof useRiftCity>}) { const factions=["Iron Syndicate","Rift Guard","Dock Union"]; return <><Panel title="Faction"><p>Faction reputation unlocks status and earns points. Membership is persistent.</p><div className="grid three">{factions.map(f=><div className={`card ${g.gameState.faction && g.gameState.faction!==f?"locked":""}`} key={f}><h3>{f}</h3><p>{g.gameState.faction===f?`Reputation: ${g.gameState.factionReputation}`:"Join for $500"}</p><Button disabled={Boolean(g.gameState.faction) && g.gameState.faction!==f} onClick={()=>g.joinFaction(f)}>{g.gameState.faction===f?"Member":"Join"}</Button></div>)}</div>{g.gameState.faction && <Button onClick={g.workFaction}>Work for faction Â· 10 â¡</Button>}</Panel></> }
