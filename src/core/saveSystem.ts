...
export function freshSave(): SaveData {
  ...
  return {
    ...
    crimeExperience: 0, crimeMastery: {}, crimeSkillXp: {}, streetReputation: 0, scoutedCrimeTargets: [], resolvedCrimeTargets: [], activeCrimeOperations: [], crimeOperationsCompleted: 0, graffitiTags: {}, graffitiCooldowns: {}, graffitiTotalTags: 0, stats: { strength: 5, defense: 5, speed: 5, dexterity: 5 },
    ...
    inventory: {}, auctionListings: [], auctionRemovedListingIds: [], productionFacilities: [], activeProductions: [], productionAttention: 0, productionBatches: 0, productionRaids: 0, equippedWeapon: null, weaponSkillXp: {}, equippedArmor: null, ownedProperty: "shack", propertyHoldings: {}, propertyRentalEnabled: {}, propertyRentEarned: 0, propertyLosses: 0, propertyLastRentAt: now, propertyRiskLastCheck: now,
...
export function loadSave(): SaveData {
  try {
    ...
    return {
      ...
      weaponSkillXp: { ...(parsed.weaponSkillXp || {}) },
      crimeMastery: { ...(parsed.crimeMastery || {}) },
      crimeSkillXp: { ...(parsed.crimeSkillXp || {}) },
      streetReputation: Math.max(0, Number(parsed.streetReputation) || 0),
      scoutedCrimeTargets: Array.isArray(parsed.scoutedCrimeTargets) ? parsed.scoutedCrimeTargets.slice(-120) : [],
      resolvedCrimeTargets: Array.isArray(parsed.resolvedCrimeTargets) ? parsed.resolvedCrimeTargets.slice(-120) : [],
      activeCrimeOperations: Array.isArray(parsed.activeCrimeOperations) ? parsed.activeCrimeOperations : [],
      crimeOperationsCompleted: Math.max(0, Number(parsed.crimeOperationsCompleted) || 0),
      graffitiTags: { ...(parsed.graffitiTags || {}) },
      graffitiCooldowns: { ...(parsed.graffitiCooldowns || {}) },
      graffitiTotalTags: Math.max(0, Number(parsed.graffitiTotalTags) || 0),
